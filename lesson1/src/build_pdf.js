...
Step(3, '試著開到磚牆上：你會被擋住！',
  '坦克碰到磚牆會被「彈回來」，沒辦法穿過去。\n這就是磚牆的用處 — 設計坦克要走的路！');

Step(4, '開坦克繞過磚牆，去碰老鷹',
  '把坦克開到金色老鷹的位置。當坦克「碰到」老鷹的時候：\n' +
  '   ▸  電腦會播放「叮~叮~叮~」勝利音效（記得開喇叭！）\n' +
  '   ▸  坦克頭上出現「我贏了！」對話框\n' +
  '   ▸  遊戲自動停下來');
...
P('現在舞台上只有 1 塊磚牆。讓我們加更多塊，做一個迷宮！\n（複製出來的磚牆 — 磚牆2、磚牆3… — 也會擋住坦克，因為它們是一樣的顏色。）');
...
P('還會教坦克不要跑出舞台外面，並且可以踩過落葉、石頭等不同地形喔。');
